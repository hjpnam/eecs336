...
int FindMeasure (const std::vector<int>& left, const std::vector<int>& right, int coverage)
{
   ...
   return FindMeasure (intervals, coverage);
}
...
int main(int argc, char *argv[])
{
   ...
   AddColumn<ProblemN5>(prAdapter, "left", &ProblemN5::left);
   AddColumn<ProblemN5>(prAdapter, "right", &ProblemN5::right);
   AddColumn<ProblemN5>(prAdapter, "coverage", &ProblemN5::coverage, -1);
   ...
   if (!isDebugMode)
   {
      for (int i = 0; i < header.problem_count; i++)
      {
         ...
         theProblem.student_answer = FindMeasure (theProblem.left, theProblem.right, theProblem.coverage);
      }
